...
Start with the filenames, before there are twenty of them: ordering is the only one of these problems that gets worse with time. For the HTML, drop the parts onto [M2H](https://md-2-html.vercel.app) together: several files at once are chained into one document, in order, separated by a rule. From a terminal, `m2h push handbook/*.md --merge --share link` prints a link to pass on.
