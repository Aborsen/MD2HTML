...
Choose by requirement: Python-Markdown for extensions and plugins, markdown2 when one call with a list of extras is the whole job, mistune when the HTML has to come out a particular shape, markdown-it-py when it must match the spec. Then write the script and pin the library in your requirements file. If all you need is a page a colleague can open, skip the build — drop the file into [M2H](https://md-2-html.vercel.app), or have the script POST it to `/api/v1/documents?share=link`, which stores the document and returns the read-only link in one call; the endpoints are in [/docs](/docs).
