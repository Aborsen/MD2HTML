/*
 * The pages that are only words: about, contact, and the legal three.
 *
 * One list, for the same reason the conversions are one list — the footer links to them, the app
 * renders them, and the prerenderer builds the static page a crawler and a search result get. A
 * second copy of this text would be the copy that goes stale, and stale text on a privacy page is
 * worse than no page at all.
 *
 * Plain strings rather than React, because the prerenderer runs in Node and has no React in it.
 */
...
export interface PageSection {
  heading: string;
  /** One paragraph each. Kept short: nobody reads a wall on a page like this. */
  body: string[];
  /** A list under the paragraphs, when the content is genuinely a list. */
  items?: string[];
}
...
export interface StaticPage {
  ...
  lede: string;
  /** When it was last changed, for the pages where a reader is entitled to know. */
  updated?: string;
